...
| Acceso para Geshe / equipo Sangha | `/account/` es el acceso de cuenta; después de iniciar sesión el rol ADMIN verá el botón **Administración · CMS**, que lleva a `/admin/`. En `/admin/students.html`, un ADMIN puede elevar una cuenta registrada a ADMIN (no puede cambiar su propio rol ni quitar al último ADMIN). |
| Subir clases/documentos | Desde `/admin/programs.html` se crea/publica el programa. Luego `/admin/content.html?new=1` permite asociar un recurso, escoger tipo/acceso/estado y cargar archivo al bucket privado `protected-content`; publicar sólo material aprobado. |
